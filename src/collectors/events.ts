...
function mapEventNode(node: unknown): EventRecord | null {
  ...
  if (eventData) {
    ...
    if (typeof data === 'string') {
      ...
      // Each element may be a plain string or an object with a #text value
      // (the latter happens when the <Data> element has a Name attribute,
      // e.g. Event 1002: <Data Name="AppName">BlackBoxHangTest.exe</Data>)
      const parts = (data as unknown[]).map((item) => {
      ...
      // Single Data element with a #text value
...
export async function collectEvents(
  ...
  const [systemResult, appResult] = await Promise.all([
    queryEventLog('System', SYSTEM_EVENT_IDS, startTime).then((r) => {
      console.log(`[collector:events] System log done in ${Date.now() - t0}ms, exitCode=${r.exitCode}, bytes=${r.stdout.length}`)
      return r
    }),
    queryEventLog('Application', APP_EVENT_IDS, startTime).then((r) => {
      console.log(`[collector:events] Application log done in ${Date.now() - t0}ms, exitCode=${r.exitCode}, bytes=${r.stdout.length}`)
      return r
    }),
  ])
