...
function EvidenceList({ signals }: { signals: SignalMatch[] }) {
  ...
  // Sort: critical first, then by proximity to marker
...
export default function Results() {
  const { navigate, resetSession, analysisResult, appName, issueType } = useStore()
  ...
  const sessionLabel = [appName, issueType].filter(Boolean).join(' — ')
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {sessionLabel && <p className="text-secondary text-sm">{sessionLabel}</p>}
            </div>
          </div>
          <button
            onClick={handleNewSession}
  ...
  // ── Diagnosed outcome ────────────────────────────────────────────────────────
  // primary_cause_name and primary_output_text are non-null when outcome === 'diagnosed'
